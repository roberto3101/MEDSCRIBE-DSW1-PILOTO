...
import { Shield, Plus, Edit2, Save, AlertTriangle, Trash2 } from 'lucide-react'
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {roles.map(rol => {
          ...
          return (
            <div key={rol.idRol} className="bg-white rounded-xl border border-slate-200 p-5">
              ...
              <div className="flex gap-2">
                ...
                {!rol.esRolBase && rol.nombreDelRol !== usuario?.rolDelSistema && (
                  <button onClick={async () => {
                    if (!confirm('Desactivar este rol? Los usuarios asignados perderan acceso.')) return
                    await fetch(`/api/roles/${rol.idRol}`, {
                      method: 'PUT',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ nombre: `${rol.nombreDelRol} (Desactivado)`, descripcion: 'Rol desactivado', permisosJson: '{}' }),
                    })
                    cargarRoles()
                  }}
                    className="flex items-center justify-center gap-1 py-2 px-3 border border-red-200 rounded-lg text-xs text-red-400 hover:bg-red-50 hover:text-red-600 transition-colors">
